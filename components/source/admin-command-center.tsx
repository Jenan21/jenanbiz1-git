...
export async function AdminCommandCenter({
  ...
  const ar = locale === "ar";
  const catalog = await readPlatformCatalog();
  const catalogSections = catalog.modules.filter(
    (catalogModule) => catalogModule.id !== "dashboard",
  );
  const catalogServiceCount = catalogSections.reduce(
    (total, catalogModule) => total + catalogModule.services.length,
    0,
  );
  return (
    <PlatformShell
    ...
    >
      ...
      <section
        className="admin-catalog glass"
        aria-labelledby="admin-catalog-title"
      >
        <header className="admin-catalog__header">
          <div>
            <span className="eyebrow">
              PLATFORM CATALOG / V{catalog.version}
            </span>
            <h2 id="admin-catalog-title">
              {ar ? "إدارة الأقسام والخدمات" : "Sections & Services Management"}
            </h2>
            <p>
              {ar
                ? "واجهة الإدارة المستقبلية لإضافة أو تحديث أو حذف الأقسام والخدمات دون تعديل بنية الصفحات."
        ...
        <div className="admin-catalog__summary">
          <span>
            <small>{ar ? "الأقسام الرئيسية" : "Main sections"}</small>
            <strong>{String(catalogSections.length).padStart(2, "0")}</strong>
          </span>
          <span>
            <small>{ar ? "الخدمات المثبتة" : "Defined services"}</small>
            <strong>{String(catalogServiceCount).padStart(2, "0")}</strong>
          </span>
          <span>
            <small>{ar ? "حالة المصدر" : "Source state"}</small>
            <strong>PREVIEW</strong>
          </span>
        </div>

        <div className="admin-catalog__sections">
          {catalogSections.map((catalogModule) => (
            <article key={catalogModule.id}>
              <span>
                <Icon name={catalogModule.icon} />
              </span>
              <div>
                <strong>{tr(locale, catalogModule.title)}</strong>
                <small>
                  {ar
                    ? `${catalogModule.services.length} خدمات`
                    : `${catalogModule.services.length} services`}
                </small>
              </div>
              <i aria-hidden="true" />
            </article>
        ...
        <footer className="admin-catalog__actions">
          <span>
            <Icon name="shield" />
            {ar
              ? "الأزرار تصميمية حتى توصيل مزود كتالوج معتمد."
              : "Controls remain design-only until an approved catalog provider is connected."}
          </span>
          <div>
            <button className="button button--secondary" type="button" disabled>
              <Icon name="plus" /> {ar ? "إضافة قسم" : "Add section"}
            </button>
            <button className="button button--primary" type="button" disabled>
              <Icon name="plus" /> {ar ? "إضافة خدمة" : "Add service"}
            </button>
          </div>
        </footer>
      </section>
