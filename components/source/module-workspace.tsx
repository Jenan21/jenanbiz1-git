...
import { ModuleShowcase } from "@/components/source/module-showcase";
...
export async function ModuleWorkspace({
  ...
  const ar = locale === "ar";
  const index = ar ? 0 : 1;
  const catalogModule = await findPlatformModule(route);
  if (route === "/projects" && catalogModule) {
    return (
      <ModuleShowcase
        locale={locale}
  ...
  if (route === "/academy" && catalogModule) {
    return (
      <AcademyCinematic
        locale={locale}
        module={catalogModule}
  ...
  const cfg: WorkspaceBlueprint = catalogModule
    ? {
        capabilities: catalogModule.services
          .slice(0, 4)
          .map(
            ({ description, icon, title }) =>
              [icon, title, description] as Capability,
          ),
        description: catalogModule.description,
        eyebrow: catalogModule.eyebrow,
        stages: catalogModule.stages,
        title: catalogModule.title,
      }
    : (workspaces[route] ?? workspaces["/dashboard"]);
  const inactive = ar
  ...
  return (
    <PlatformShell locale={locale} activeRoute={route} userLabel={userLabel}>
      <div className="module-cinema">
        <ModuleIdentityRail locale={locale} route={route} />
        <section className="module-hero glass">
          ...
          <div className="module-hero__network" aria-hidden="true">
            <ModuleIdentityScene route={route} />
          </div>
        </section>
      </div>
